...
impl SoundGenerator {
    ...
    pub fn _set_sound(&mut self, sound: Sound) {
        self.sound = sound;
    }
    ...
    pub fn play(&self, value: u32) -> Result<()> {
        ...
        primary.frequency().set_value(self.frequency_from_value(value));

        // Give the amp a shape.
        gain.gain().set_value(0.0);
        gain.gain().linear_ramp_to_value_at_time(sound.peak, self.ctx.current_time() + sound.attack)?;
        gain.gain().linear_ramp_to_value_at_time(0.0, self.ctx.current_time() + sound.sweep - sound.release)?;
    ...
    fn midi_note_from_value(&self, value: u32) -> u8 {
    ...
    fn frequency_from_value(&self, value: u32) -> f32 {
